...
namespace
{
    ...
    struct PipelineLibrary
    {
        ...
        {
            std::shared_ptr<Wrapper> pipeline_first = wif();
            std::shared_ptr<Wrapper> pipeline_second = base64();
            std::shared_ptr<Wrapper> pipeline_third = stdout();

            pipeline_second->link_to(pipeline_third.get());
            pipeline_first->link_to(pipeline_second.get());
