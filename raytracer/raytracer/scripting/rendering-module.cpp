...
namespace
{
    struct RendererLibrary
    {
        ...
        Renderer naive_parallel(unsigned width, unsigned height, Sampler sampler, RayTracer ray_tracer) const
        {
            return raytracer::renderers::standard(width, height, sampler, ray_tracer, tasks::schedulers::naive_parallel());
        }

        Renderer naive_parallel_by_map(const std::map<std::string, Boxed_Value>& argument_map) const
        {
            START_ARGUMENTS(argument_map);
            ARGUMENT(unsigned, width);
            ARGUMENT(unsigned, height);
            ...
            return naive_parallel(width, height, sampler, ray_tracer);
        }
    };
}
...
#   define BIND_AS(INTERNAL, EXTERNAL) module->add(fun(&RendererLibrary::INTERNAL), #EXTERNAL)
    ...
    BIND_AS(naive_parallel, naive_parallel);
    BIND_AS(naive_parallel_by_map, naive_parallel);
#   undef BIND_AS
