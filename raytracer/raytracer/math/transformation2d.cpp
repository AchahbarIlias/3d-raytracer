...
Transformation2D math::transformations::rotate(const Angle angle)
{
...
Transformation2D math::transformations::scale(const double sx, const double sy)
{
    Matrix3x3 tm = transformation_matrices::scaling(sx, sy);
    Matrix3x3 itm = transformation_matrices::scaling(sx, sy);
    return Transformation2D(tm, itm);
}
